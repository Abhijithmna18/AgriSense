...
import { createInitialNegotiationState } from '../types/negotiation.js';
import { negotiationApi } from '../services/negotiationApi.js';
import { negotiationValidator } from '../utils/negotiationValidation.js';

export const useNegotiation = (negotiationId) => {
  const [state, setState] = useState(createInitialNegotiationState());
  ...
  // Create new offer
  const createOffer = useCallback(async (offerForm) => {
    ...
    try {
      ...
      const response = await negotiationApi.createOffer(state.negotiation.id, offerForm);
  ...
  // Reject offer
  const rejectOffer = useCallback(async (offerId, form) => {
    try {
  ...
  // Counter offer
  const counterOffer = useCallback(async (offerId, offerForm) => {
    try {
      // Validate offer
      const validation = negotiationValidator.validateCreateOffer(
        offerForm,
  ...
  // Upload attachment
  const uploadAttachment = useCallback(async (offerId, file) => {
    try {
      // Validate file
      ...
      const response = await negotiationApi.uploadAttachment(offerId, file);
  ...
  const setActiveOffer = useCallback((offer) => {
  ...
  const canCreateNewOffer = useCallback(() => {
